  ...
  public class HomeController : Controller
  {
    ...
    public ActionResult ForgotPasswordCustomer(string Email)
    {
      ...
      if (usercount > 0)
      {
        ...
        //==================SEND MAIL============================//
        string title = "Password Reset Code!";
        string msg = "<center><a href='https://xamagos.com' title='Xamagos'> <img src='https://xamagos.com/Images/logosquare.png' style='width: 70px; height: 30px;background-color:#17A2B8' /></a></center><br>" + "<strong>" + " Hello " + Email + "</strong>, <br><br>";
        msg += "Your verification code is: <b>" + RegisterOTP_Numeric + "</b> <br>Please enter this code to reset your password on XAMAGOS.";
        msg += "<br>This code expires in 10 minutes. <br> Do not share this code with anyone. Thank you!.";
        msg += "<br><hr>";
        msg += "<a href='https://xamagos.com'>www.xamagos.com</a> &nbsp;&nbsp;<a href='https://tinyurl.com/23a8r43k'>Get support</a>";
        ...
        ViewBag.MailSent = "An OTP has been sent to your email. Enter the OTP to reset your password.";
    ...
    {
      //int id = 1;
      //var update = db.Set<Setting>().Find(id);
      //update.TnC =Convert.ToString(Request.Form["terms"]);
      //db.SaveChanges();
      return Redirect("~/Home/TnC");
    }


    public JsonResult ForgotPasswordAjax(string email)
    {
      if (db.Users.Where(s => s.Email == email && s.EmailVerify == 1).Count() == 1)
      {
        ...
        msg += "<a href='https://xamagos.com'>www.xamagos.com</a> &nbsp;&nbsp;<a href='https://tinyurl.com/23a8r43k'>Get support</a>";
        mydata.SendMail(email, title, msg);
        var result =new { mailsent = "true"};
        return Json(result, JsonRequestBehavior.AllowGet);
      }
      else
      {
        var result = new { mailsent = "false" };
        return Json(result, JsonRequestBehavior.AllowGet);
      }
    }


    public JsonResult ResetPasswordAjax(string email, string password,string otp)
    {
      if (Convert.ToString(Session["OTP_Resetpw"]) == otp)
      {
          int userID = db.Users.Where(a => a.Email == email).Select(a => a.ID).FirstOrDefault();
          var u = db.Set<User>().Find(userID);
          u.Password = Crypto.HashPassword(password);
          db.SaveChanges();
          var result = from p in db.Users
                       where p.Email == email
                       select new { status = "true", userid = p.ID };
          return Json(result, JsonRequestBehavior.AllowGet);
        }
        else
        {
          ...
          var result = from p in db.Users
                       where p.Email == email
                       select new { status = "false", userid = p.ID };
          return Json(result, JsonRequestBehavior.AllowGet);
        }
      }
